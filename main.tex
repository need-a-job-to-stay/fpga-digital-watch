...
\section{Stopwatch}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.0\linewidth]{figures/report10.png}
...
\section{Synthesis}
	...
	\end{reportbox}
		\begin{figure}[H]
			...
			\caption{Graphviz representation of design for texttt{mod\_n\_counter}.}
		\end{figure}
	...
	\end{reportbox}
		\begin{figure}[H]
			...
			\caption{proc.png of design for \texttt{mod\_n\_counter}.}
		\end{figure}
...
\section{Timer clearer and Preset Timer Interval}
	\subsection{Specification}
	This function is accessable only in Timer Mode. 
	\begin{itemize}
			\item In edit mode: Pressing of \texttt{button[2]} reset time to zero.
			\item When timer is running: Pressing \texttt{button[2]} is equivalent to a press of \texttt{button[0]} that stops the timer. If both buttons are pressed at the same time, the timer stop.
			\item When timer is stopped with remaining time: ignore \texttt{button[2]}
			\item When timer is stopped with no remaining time: A press of \texttt{button[2]} jumps to different preset intervals in order then finally back to 0. Every time the first default interval is 30 seconds then follow by others. If the timer has time greater than zero, timer can be started by pressing \texttt{button[0]}.
			\item Default interval: 30 seconds; 1 minute; 5 minutes; 25 minutes
		\end{itemize}
	Press of \texttt{button[2]} is detected by \texttt{rising\_edge\_detector}. Any identified presses from both button whose single-cycle-pulse generated from detector that happened in different cycles is treated as independent event and will change the timer behaviour if it is a valid input in the current state. 
	\textbf{State Diagram}
	Block \texttt{1 N} is the representation of all possible running timer.
	\begin{figure}[H]
			\centering
			\includegraphics[width=1.0\linewidth]{figures/report20_1.png}
			\caption{State diagram for preset timer interval selection.}
	\end{figure}


\subsection{Implementation Logic}
	Preset timer intervals are implemented by adding a new module \texttt{editable\_countdown\_preset} based on \texttt{editable\_countdown}. New input signals \texttt{load} and \texttt{load\_value} are added so preset values can be synchronously loaded into the timer counters. Hours, minutes, and seconds counters all instantiate this modified module.\\
	\\
	Preset interval selection is implemented using \texttt{mod\_n\_counter}. Rising edge of \texttt{button[2]} generated by \texttt{rising\_edge\_detector} increments \texttt{default\_selection}. Combinational logic maps \texttt{default\_selection} into corresponding preset hours, minutes, and seconds values.\\
	\\
	Additional decision logic determines behaviour of \texttt{button[2]}. In edit mode, \texttt{button[2]} resets timer to zero. While timer is running, \texttt{button[2]} behaves like stop input. When timer is stopped at zero, \texttt{button[2]} enters preset selection behaviour and cycles through preset intervals.\\
	\\
	\texttt{preset\_load\_d} is generated using an \texttt{always\_ff} block because \texttt{default\_selection} updates on the same clock edge as preset loading. Delaying load signal by one cycle ensures newly selected preset value is loaded correctly instead of previous value.

\subsection{Design Reasoning}
	Extending the existing timer structure is more efficient than making another separated preset controller. Existing countdown chain, borrow logic, and display logic already worked correctly so modifying the counter module is less likely to break timer behaviour during integration.\\
	\\
	\texttt{mod\_n\_counter} is used for preset selection because preset intervals naturally behave like cyclic states. The counter automatically wraps from 25 minutes back to 0 without extra FSM logic.\\
	\\
	Preset values are generated by combinational logic instead of memory because only several fixed intervals are needed. It is easier to directly map \texttt{default\_selection} into corresponding hours, minutes, and seconds values.\\
	\\
	\texttt{preset\_load\_d} is added because \texttt{default\_selection} and preset loading both happen at the same clock edge. Without delaying load by one cycle, timer would load the previous preset value instead of the newly selected one.

\subsection{Testbench}
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.0\linewidth]{figures/user_top_timer_add_preset1.png}
		\caption{Waveform showing preset interval selection and countdown.}
	\end{figure}
	Pressing \texttt{button[2]} when timer is stopped at zero changes \texttt{default\_selection} in the sequence 1, 2, 3, 4, then back to 0. The corresponding display values change to 30 seconds, 1 minute, 5 minutes, 25 minutes, and finally 0.\\
	\\
	\texttt{preset\_load} rises when \texttt{button[2]} is detected in preset mode. Since \texttt{default\_selection} updates at the clock edge, \texttt{preset\_load\_d} delays the load signal by one cycle, so the newly selected interval is loaded instead of the previous one.\\
	\\
	When \texttt{button[0]} is pressed, \texttt{running} becomes high and the timer starts counting down. At the same time, \texttt{default\_selection} resets to 0. This makes the next preset selection start from 30 seconds again.\\
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.0\linewidth]{figures/same_time_press.png}
		\caption{Waveform showing button[2] and button[0] press at the same time.}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.0\linewidth]{figures/edit_reset.png}
		\caption{Waveform showing reset of time in edit mode.}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.0\linewidth]{figures/ignore2.png}
		\caption{Waveform showing button[2] reation.}
	\end{figure}	
	When timer is stopped with remaining time, \texttt{button[2]} does not trigger any output or state.
...
\section{Reflection}
	...
	\end{reportbox}
	Some parts of a digital watch are very well suited to FPGA hardware because they naturally operate in parallel and depend on precise timing. Counters, clock dividers, PWM brightness control, and seven-segment display driving are all simple synchronous circuits that map directly onto flip-flops and combinational logic. For example, the stopwatch, timer, and watch counters all run concurrently without needing a scheduler or operating system. The FPGA also provides deterministic timing behaviour: at 50 MHz, every clock edge occurs at a fixed interval, which makes implementing accurate tick generation straightforward. PWM brightness control is also naturally hardware-oriented because it only requires counters and combinational comparisons running continuously in parallel. The synthesis investigation with Yosys showed this clearly: behavioural Verilog was reduced into flip-flops, XOR gates, NOT gates, and multiplexers representing the actual hardware structure.\\
	\\
	Hardware is also advantageous for real-time signal handling. Button presses, edge detection, debouncing, and auto-repeat logic can all operate simultaneously every clock cycle with extremely low latency. The FPGA continuously evaluates all logic in parallel, unlike software which executes instructions sequentially. This became especially clear during waveform debugging and formal verification, where each module behaved like an independent hardware machine instead of a sequence of instructions.\\
	\\
	However, some parts would be easier in software. Complex control logic and user interaction are generally simpler to express procedurally. For example, the stopwatch and timer FSMs required careful reasoning about states, transitions, enable conditions, and one-cycle pulses. In software, these behaviours could often be implemented with a few variables and if-statements inside a loop. Features such as mode switching, menu systems, alarms, or storing configuration values are usually easier to write and modify in software because software naturally supports abstraction, dynamic data structures, and sequential control flow.\\
	\\
	Debugging can also be easier in software. In FPGA design, bugs may arise from timing interactions, race conditions, width mismatches, or synthesis behaviour differing from simulation. The project repeatedly emphasised the distinction between simulation and synthesis. Hardware debugging often required waveform inspection and formal verification to locate faults. In contrast, software debugging typically provides easier visibility into variables and execution flow.\\
	\\
	Overall, FPGA hardware is strongest for deterministic, highly parallel, timing-critical components such as counters and display driving, while software is generally more convenient for complex control flow, user interfaces, and high-level behaviour. A real embedded product would often combine both approaches: dedicated hardware peripherals for timing-critical tasks and software running on a processor for overall system control.\\
	\\

...
\begin{figure}[H]
	...
	\caption{Hardware inplementation diagram for report 9.}
\end{figure}
\begin{reportbox}{Report 10}
...
\end{reportbox}
Initial state should not has rst high. In final design the state is defined by {runinng or not, live display or not, only\_lap\_rise or not}. clk should be consider as an input that effects state too because rst is a one cycle event then the state shift back.
\section{Timer}
Done.
